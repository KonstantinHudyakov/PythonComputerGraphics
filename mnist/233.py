...
def gen_images(datagen, data, labels, n):
    batch_size = 200
    images = datagen.flow(data, labels, batch_size=batch_size)
    iterations = n // batch_size
    gen_data, gen_labels = images[0]
    for i in range(1, iterations):
        part_data, part_labels = images[i]
        gen_data = np.concatenate((gen_data, part_data))
        gen_labels = np.concatenate((gen_labels, part_labels))
    return gen_data, gen_labels


def increase_dataset(datagen, data, labels):
    gen_data, gen_labels = gen_images(datagen, data, labels, len(data))
...
x_train, y_train = increase_dataset(datagen, x_train, y_train)
x_test, y_test = increase_dataset(datagen, x_test, y_test)

images = datagen.flow(x_train, y_train, batch_size=30)
images = images[0][0]
k = -1
